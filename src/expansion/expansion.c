...
/*   Updated: 2025/07/08 09:56:38 by odana            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	handle_empty_token(t_token **tokens, t_token *current, t_token *prev)
{
	...
	if (ft_strlen(current->value) == 0 && !current->single_quotes 
		&& !current->double_quotes)
	{
								...
								t_token *prev)
{
	...
	if (current && current->has_wildcard && !current->single_quotes 
		&& !current->double_quotes)
	{
